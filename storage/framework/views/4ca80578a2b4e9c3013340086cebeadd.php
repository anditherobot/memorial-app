...
<div class="max-w-7xl mx-auto space-y-6">
  ...
  <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
    <?php $__currentLoopData = ['todo' => 'To Do', 'in_progress' => 'In Progress', 'blocked' => 'Blocked', 'completed' => 'Completed']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $status => $label): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
      <div class="kanban-col bg-white/80 border rounded-xl p-4">
        <h2 class="font-semibold text-gray-700 mb-4 flex items-center justify-between sticky top-0 bg-white/80 backdrop-blur rounded px-1 py-1">
          ...
          <span class="count-pill">(<?php echo e($tasks->get($status, collect())->count()); ?>)</span>
        </h2>

        <div class="space-y-3 task-column" data-status="<?php echo e($status); ?>">
          <?php $__currentLoopData = $tasks->get($status, collect()); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $task): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <?php
            ...
            ?>
            <div class="task-card task-card-ui bg-white border rounded-lg p-3 shadow-sm cursor-pointer hover:shadow-md transition-shadow border-l-4 <?php echo e($border); ?>"
                 data-task-id="<?php echo e($task->id); ?>"
                 data-status="<?php echo e($task->status); ?>"
                 data-task='<?php echo json_encode($task->toArray(), 15, 512) ?>'>
              <div class="flex items-start justify-between mb-2">
                <h3 class="font-medium text-sm text-gray-900 leading-tight"><?php echo e($task->title); ?></h3>
                <span class="text-xs px-2 py-1 rounded <?php echo e($task->priority_color); ?> flex-shrink-0 ml-2">
              ...
              <?php if($task->description): ?>
                <p class="text-xs text-gray-600 mb-2 line-clamp-2"><?php echo e(Str::limit($task->description, 80)); ?></p>
              <?php endif; ?>

              <div class="flex items-center justify-between text-xs text-gray-500">
                <div>
                  <?php if($task->category): ?>
                    <span class="chip bg-gray-100"><?php echo e($task->category); ?></span>
                  <?php endif; ?>
                </div>
                <div class="flex items-center space-x-2">
                  <?php if($task->assignedTo): ?>
                    <span class="chip bg-gray-100"><?php echo e($task->assignedTo->name); ?></span>
                  <?php endif; ?>
                  <?php if($task->due_date): ?>
                    <?php $overdue = $task->due_date->isPast() && $task->status !== 'completed'; ?>
                    <span class="chip <?php echo e($overdue ? 'bg-red-100 text-red-700' : 'bg-gray-100'); ?>"><?php echo e($task->due_date->format('M j')); ?></span>
                  <?php endif; ?>
                </div>
              </div>

              <div class="flex items-center justify-between mt-2">
                <span class="status-badge text-xs px-2 py-1 rounded <?php echo e($task->status_color); ?>"><?php echo e(ucfirst(str_replace('_',' ', $task->status))); ?></span>
                <select class="task-status-select text-xs border rounded px-2 py-1"
                  ...
                  <option value="blocked" <?php echo e($task->status==='blocked' ? 'selected' : ''); ?>>Blocked</option>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\expan\Desktop\memorial\resources\views/admin/tasks/index.blade.php ENDPATH**/ ?>