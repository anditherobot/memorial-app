...
<div class="p-6 bg-white border rounded-lg shadow-sm space-y-4">
    <form method="POST" action="{{ $action }}" enctype="multipart/form-data">
        @csrf
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">{{ $title ?? 'Upload File' }}</label>
                ...
                <div class="space-y-1 text-center">
                    ...
                    <div class="flex text-sm text-gray-600">
                        <label for="file-upload" class="relative cursor-pointer bg-white rounded-md font-medium text-indigo-600 hover:text-indigo-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-indigo-500">
                            ...
                            <input id="file-upload" name="{{ $inputName ?? 'file' }}" type="file" accept="{{ $acceptedFileTypes ?? 'image/*' }}" required class="sr-only" onchange="previewImage(this)" />
                        </label>
                        <p class="pl-1">or drag and drop</p>
                    </div>
                    <p class="text-xs text-gray-500">PNG, JPG, GIF up to {{ $maxFileSizeMb ?? '10' }}MB</p>
                </div>
            </div>
            @error($inputName ?? 'file')
