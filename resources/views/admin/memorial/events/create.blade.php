...
@section('content')
  <div class="max-w-4xl mx-auto">
    <div class="flex items-center justify-between mb-6">
      <h1 class="text-2xl font-semibold text-gray-900">Create Memorial Event</h1>
      <a href="{{ route('memorial.events.index') }}" class="text-gray-600 hover:text-gray-900">
        ← Back to Events
      </a>
    </div>

    <form method="POST" action="{{ route('memorial.events.store') }}" enctype="multipart/form-data" class="space-y-6">
      @csrf
        ...
        <div class="p-6 space-y-6">
          ...
          <div>
            ...
            <select name="event_type" id="event_type" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500" required>
              ...
              @foreach($eventTypes as $key => $name)
                <option value="{{ $key }}" {{ old('event_type', request('type')) == $key ? 'selected' : '' }}>
                  {{ $name }}
                </option>
              @endforeach
            </select>
          ...
          <div>
            ...
            <textarea name="address" id="address" rows="3"
                      class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                      placeholder="Full address including city, state, and zip code">{{ old('address') }}</textarea>
            @error('address')
              <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
          </div>
          ...
          <div>
            ...
            <textarea name="notes" id="notes" rows="4"
                      class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                      placeholder="Special instructions, directions, or additional information...">{{ old('notes') }}</textarea>
            @error('notes')
              <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
          </div>
          ...
          <!-- Event Poster -->
          <div>
            <label for="poster" class="block text-sm font-medium text-gray-700 mb-2">Event Poster (Optional)</label>
